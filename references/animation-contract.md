...
Use Elsie's existing tempo as the default for new Officer work/greeting: about one second of advancing poses plus a two-second extra hold on frame 8, giving roughly a three-second loop. The hold length remains configurable when requested. Record the pause through frame duration, not duplicate atlas cells; frame 8's duration includes its normal frame time plus the extra hold. This extends the frame-8 policy beyond Elsie; older documents describing it as solely her setting are historical. It does not impose Elsie's idle/end-frame hold on every Officer or add a frame-8 pause to walking or combat.
...
Sprite-gen's short-action default is four frames and humanoid walk/run remain experimental until motion QA passes. Those are tooling reliability notes, not IMC's art specification. A 16-cell sheet may still contain duplicate or incoherent poses. If a requested count fails, record the failure and improve the source motion or propose an alternative; do not silently lower the count, duplicate poses as filler or present interpolated/repeated frames as 16 authored poses. Intentional held poses are valid when recorded as timing choices.
...
- Stable scale and feet/pivots, appropriate ground contact, alternating support for locomotion and no unexplained foot sliding.
